...
# 한글등 non-ASCII text를 URL에 넣을 수 있도록 "%" followed by hexadecimal digits로 변경
...
# urllib.request.urlopen 메소드로 크롤링할 웹페이지를 가져옴
response = urllib.request.urlopen(request)

# getcode() 메소드로 HTTP 응답 상태 코드를 가져올 수 있음
...
if(rescode == 200):
    # response.read() 메소드로 수신된 HTML 데이터를 가져올 수 있음
    response_body = response.read()

    # 네이버 Open API를 통해서 수신된 데이터가 JSON 포맷이기 때문에,
    # JSON 포맷 데이터를 파싱해서 사전데이터로 만들어주는 json 라이브러리를 사용
    data = json.loads(response_body)

    # json.loads() 메소드를 사용해서 data에 수신된 데이터를 사전 데이터로 분석해서 자동으로 만들어줌

...
while True :
    ...
    if message == '4': # 4를 입력하면 IF-SC400 패킷 송신
        ...
        for i in range(data_objNum) :
            data_ID = f'2021-06-18 {cctvID:02d} {i:05d}' # ID 문자열은 날짜 + CCTV ID + tracking ID로 해주시면 됩니다. 
            ...
            log = str(i) + ' : ' + str(data_ID) + ' ' + str(data_objType) + ' ' + str(data_XAxis) + ' ' + str(data_ZAxis) + ' '
            ...
            IF_SC400_ID = bytes(data_ID, encoding='UTF-16')
            ...
            sendBuffer += IF_SC400_ID + IF_SC400_objType + IF_SC400_XAxis + IF_SC400_ZAxis
